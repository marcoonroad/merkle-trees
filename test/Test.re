...
let __deterministic_generation = () => {
  ...
  Alcotest.(check(string))("should compute the same root given same leaves", root_a, root_b);
};
...
let __deterministic_path_computation = () => {
  ...
  Alcotest.(check(list(string)))("should compute the same path given same leaf", path_a, path_b);
};
...
let __deterministic_verification = () => {
  ...
  Alcotest.(check(bool))("should pass verification given proper leaf and path", true, success);
  Alcotest.(check(bool))("should fail verification given wrong leaf or path", false, failure);
};

let suite = [
  ("deterministic merkle tree generation", `Quick, __deterministic_generation),
  ("deterministic authentication path computation", `Quick, __deterministic_path_computation),
  ("deterministic merkle tree path verification", `Quick, __deterministic_verification)
];

Alcotest.run ("merkle trees specification", [("merkle trees test suite", suite)]);
